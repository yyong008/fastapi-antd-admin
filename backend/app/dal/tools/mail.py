...
def get_mail_list(db: Session, page: int = 1, pageSize: int = 10):
    ...
    # sort_column = Mail.createdAt.desc()
    return db.query(Mail).offset(offset).limit(limit).all()
...
def delete_mail_by_ids(ids, db):
    try:
        count = db.query(Mail).filter(Mail.id.in_(ids)).delete(synchronize_session=False)
        db.commit()
        return count
