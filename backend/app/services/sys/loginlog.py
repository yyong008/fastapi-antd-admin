...
async def get_loginlog_list(db: AsyncSession, page, pageSize):
    try:
...
async def create_loginlog_by_userId_name(db: AsyncSession, request: Request, name, userId):
    try:
        loginlog = get_loginlog_info(request, name, userId)
        loginlog["login_at"]= datetime.fromisoformat(loginlog["login_at"]) # 时间字符串转datetime
        data = await loginlog_dal.create_loginlog(db, loginlog)
        return format_loginlog(data)
    except Exception as e:
        print(f"Oops, we encountered an error: {e}")
